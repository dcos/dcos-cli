...
from dcos.cli.subcommand import default_command_info, default_doc
from dcos.cli.util import decorate_docopt_usage
...
def _main(argv):
    args = docopt.docopt(
        ...
        argv=argv,
        version='dcos-experimental version {}'.format(dcos.cli.version))
...
def _build(output_json,
    ...
    build_definition_schema = util.load_jsons(
        pkg_resources.resource_string(
            "dcos.cli", build_definition_schema_path).decode())
    ...
    metadata_schema = util.load_jsons(
        pkg_resources.resource_string(
            "dcos.cli", metadata_schema_path).decode())
    ...
    # create the manifest
    manifest_json = {
        'built-by': "dcos.cli.version={}".format(dcos.cli.version)
    }
