...
def test_add_bad_json_app():
    with open(_DATA_DIR + 'marathon/apps/bad.json') as fd:
...
def test_add_existing_app():
    with _zero_instance_app():
        app_path = _DATA_DIR + 'marathon/apps/zero_instance_sleep_v2.json'
...
def test_show_absolute_app_version():
    with _zero_instance_app():
        _update_app(
            'zero-instance-app',
            _DATA_DIR + 'marathon/apps/update_zero_instance_sleep.json')
...
def test_app_locked_error():
    with app(_DATA_DIR + 'marathon/apps/sleep_many_instances.json',
...
def _zero_instance_app():
    with app(_DATA_DIR + 'marathon/apps/zero_instance_sleep.json',
             'zero-instance-app'):
        yield
    ...
    class JSONRequestHandler (BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            self.wfile.write(open(
                _DATA_DIR + 'marathon/apps/zero_instance_sleep.json',
                'rb').read())
