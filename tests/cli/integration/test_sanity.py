...
_ZERO_INSTANCE_APP = _DATA_DIR + 'marathon/apps/zero_instance_sleep.json'
...
def test_add_group():
    group_id = 'test-group'
    with group(_DATA_DIR + 'marathon/groups/good.json', group_id):
        show_group(group_id)


def test_update_group():
    group_app = _DATA_DIR + 'marathon/groups/good.json'
...
def test_add_pod():
    pod_id = 'good-pod'
    with pod(_DATA_DIR + 'marathon/pods/good.json', pod_id):
        expected = file_json_ast(_DATA_DIR + 'marathon/pods/good_status.json')
        show_pod(pod_id, expected)


def test_repo_list():
    repo_list = file_json(
        _DATA_DIR + 'package/json/test_repo_list.json')
    assert_command(
...
def test_package_describe():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_marathon.json')
