...
def test_describe_cli():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_cli_kafka.json')
...
def test_describe_app():
    stdout = file_bytes(
        _DATA_DIR + 'package/json/test_describe_app_marathon.json')
...
def test_describe_config():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_marathon_config.json')
...
def test_describe_render():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_marathon_app_render.json')
...
def test_describe_package_version():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_marathon_package_version.json')
...
def test_describe_package_versions():
    stdout = file_bytes(
        _DATA_DIR + 'package/json/test_describe_kafka_package_versions.json')
    assert_command(
...
def test_describe_options():
    stdout = file_json(
        _DATA_DIR + 'package/json/test_describe_app_options.json')
    stdout = json.loads(stdout.decode('utf-8'))
    expected_labels = stdout.pop("labels", None)

    returncode, stdout_, stderr = exec_command(
        ['dcos', 'package', 'describe', '--app', '--options',
         _DATA_DIR + 'package/marathon.json', 'marathon'])
...
def test_describe_app_cli():
    stdout = file_bytes(
        _DATA_DIR + 'package/json/test_describe_app_cli.json')
    assert_command(
...
def test_bad_install():
    args = ['--options=' + _DATA_DIR + 'package/chronos-bad.json', '--yes']
    stderr = """\
...
def test_package_metadata():
    ...
    package = file_json(
        _DATA_DIR + 'package/json/test_package_metadata.json')
...
def test_list(zk_znode):
    ...
    with _chronos_package():
        ...
        expected_output = file_json(
            _DATA_DIR + 'package/json/test_list_chronos.json')
...
def test_install_yes():
    with open(_DATA_DIR + 'package/assume_yes.txt') as yes_file:
        _install_helloworld(
            args=[],
            stdin=yes_file,
            stdout=(
...
def test_install_no():
    with open(_DATA_DIR + 'package/assume_no.txt') as no_file:
        _install_helloworld(
            args=[],
            stdin=no_file,
            stdout=(
...
def test_list_cli():
    _install_helloworld()
    stdout = file_json(
        _DATA_DIR + 'package/json/test_list_helloworld.json')
    ...
    stdout = file_json(
        _DATA_DIR + 'package/json/test_list_helloworld_cli.json')
    _list(args=['--json'], stdout=stdout)
...
def test_list_cli_only(env):
    helloworld_path = _DATA_DIR + 'package/json/test_list_helloworld_cli.json'
...
def test_uninstall_multiple_frameworknames(zk_znode):
    _install_chronos(
        args=['--yes', '--options=' + _DATA_DIR + 'package/chronos-1.json'])
    _install_chronos(
        args=['--yes', '--options=' + _DATA_DIR + 'package/chronos-2.json'])

    watch_all_deployments()

    expected_output = file_json(
        _DATA_DIR + 'package/json/test_list_chronos_two_users.json')

    _list(args=['--json'], stdout=expected_output)
    _list(args=['--json', 'chronos'], stdout=expected_output)
    _list(args=['--json', '--app-id=/chronos-user-1'], stdout=file_json(
        _DATA_DIR + 'package/json/test_list_chronos_user_1.json'))

    _list(args=['--json', '--app-id=/chronos-user-2'], stdout=file_json(
        _DATA_DIR + 'package/json/test_list_chronos_user_2.json'))
