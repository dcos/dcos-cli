...
import dcos.cli.marathon.main as main
...
def test_pod_list_with_json():
    id_array = [{'id': 'one'}, {'id': 'two'}, {'id': 'three'}]
    _assert_pod_list_with_json(pod_list_json=id_array)
    _assert_pod_list_with_json(pod_list_json=[{'id': 'a'}, {'id': 'b'}])


@patch('dcos.cli.marathon.main.emitter', autospec=True)
def test_pod_list_table(emitter):
    ...
    expected_table = file_bytes('tests/cli/unit/data/pod.txt')
